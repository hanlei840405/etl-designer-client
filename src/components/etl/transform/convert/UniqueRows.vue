  ...
  <div style="width: 100%;">
    <q-form class="q-gutter-md">
      <q-tabs v-model="tab" align="left" narrow-indicator>
        <q-tab name="basic" :label="$t('form.uniqueRows.tabBasic')"/>
        <q-tab name="runningConfig" :label="$t('form.uniqueRows.tabRunningConfig')"/>
      </q-tabs>
      <q-tab-panels v-model="tab" animated>
        <q-tab-panel class="row q-col-gutter-xs" name="basic">
          <q-input class="col-12 col-md-6" outlined v-model="form.name" :label="$t('form.uniqueRows.name')" :rules="[ val => val && val.length > 0 || 'Please type something']" hint=""/>
          <q-input class="col-12 col-md-6" outlined v-model="form.counterName" :label="$t('form.uniqueRows.couterField')" :disable="!form.useCounter" hint=""/>
          <q-input class="col-12 col-md-6" outlined v-model="form.error" :label="$t('form.uniqueRows.errorDescription')" :disable="!form.useRedirect" hint=""/>
          <q-checkbox class="col-12 col-md-3" outlined v-model="form.useCounter" :label="$t('form.uniqueRows.addCouter')"/>
          <q-checkbox class="col-12 col-md-3" outlined v-model="form.useRedirect" :label="$t('form.uniqueRows.redirectDuplicateRow')"/>
          <q-table class="col-12" :data="form.parameters" :columns="parameterColumns" :rows-per-page-options="[0]" row-key="name" separator="cell" hide-bottom :title="$t('form.uniqueRows.tableField')">
              ...
              <q-tr :props="props">
                ...
                <q-td key="ignore" :props="props">
                  ...
                  <q-popup-edit v-model="props.row.ignore" :auto-save="true">
                    <q-select autofocus outlined v-model="props.row.ignore" :options="ignores"/>
                  </q-popup-edit>
                </q-td>
              </q-tr>
            </template>
        ...
        <q-tab-panel name="runningConfig">
          <q-input outlined v-model.number="form.parallel" :label="$t('form.uniqueRows.number-thread-copies')" type="number" min="1" :disable="forbiddenParallel"/>
        </q-tab-panel>
      </q-tab-panels>
  </q-form>
  </div>
  ...
  data () {
    return {
      ...
      parameterColumns: [
        {
          name: 'operate',
          label: this.$t('form.uniqueRows.columnOperate'),
          field: 'operate',
          align: 'right',
        ...
        {
          name: 'field',
          label: this.$t('form.uniqueRows.columnSourceField'),
          field: 'field',
          align: 'left',
        ...
        {
          name: 'ignore',
          label: this.$t('form.uniqueRows.columnIgnore'),
          field: 'ignore',
          align: 'left',
          headerStyle: 'width: 100px;'
        }
  ...
  methods: {
    ...
    submitForm () {
      this.$emit('propertiesForm', {
        ...
        ext: { sourceFields: this.form.useCounter ? [this.form.counterName] : [] }
      })
    }
  },
  mounted () {
