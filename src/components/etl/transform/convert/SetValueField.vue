  ...
  <div style="width: 100%;">
    <q-form class="q-gutter-md">
      <q-tabs v-model="tab" align="left" narrow-indicator>
        <q-tab name="basic" :label="$t('form.setValueField.tabBasic')"/>
        <q-tab name="runningConfig" :label="$t('form.setValueField.tabRunningConfig')"/>
      </q-tabs>
      <q-tab-panels v-model="tab" animated>
        <q-tab-panel name="basic">
          <q-input outlined v-model="form.name" :label="$t('form.setValueField.name')" :rules="[ val => val && val.length > 0 || 'Please type something']" hint=""/>
          <q-table :data="form.parameters" :columns="parameterColumns" :rows-per-page-options="[0]" row-key="name" separator="cell" hide-bottom :title="$t('form.setValueField.tableField')">
              ...
              <q-tr :props="props">
                ...
                <q-td key="target" :props="props">
                  ...
                  <q-popup-edit v-model="props.row.target" :auto-save="true">
                    <q-select autofocus outlined v-model="props.row.target" :options="sourceFields" @new-value="createSourceField" use-input/>
                  </q-popup-edit>
                </q-td>
              </q-tr>
            </template>
        ...
        <q-tab-panel name="runningConfig">
          <q-input outlined type="number" v-model.number="form.parallel" :label="$t('form.setValueField.threads')" min="1"
  ...
  data () {
    return {
      ...
      parameterColumns: [
        {
          name: 'operate',
          label: this.$t('form.setValueField.columnOperate'),
          field: 'operate',
          align: 'right',
        ...
        {
          name: 'source',
          label: this.$t('form.setValueField.columnSourceField'),
          field: 'source',
          align: 'left',
        ...
        {
          name: 'target',
          label: this.$t('form.setValueField.columnTargetField'),
          field: 'target',
          align: 'left',
          headerStyle: 'width: 100px;'
        }
