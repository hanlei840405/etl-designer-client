  ...
  <div style="width: 100%;">
    <q-form class="q-gutter-md">
      <q-tabs v-model="tab" align="left" narrow-indicator>
        <q-tab name="basic" :label="$t('form.tableInsertUpdate.tabBasic')" />
        <q-tab name="lookup" :label="$t('form.tableInsertUpdate.tabLookupCondition')" />
        <q-tab name="field" :label="$t('form.tableInsertUpdate.tabField')" />
        <q-tab name="runningConfig" :label="$t('form.tableInsertUpdate.tabRunningConfig')"/>
      </q-tabs>
      <q-separator />
      <q-tab-panels v-model="tab" animated>
        <q-tab-panel class="row q-col-gutter-xs" name="basic">
          <q-input class="col-12 col-md-6" outlined v-model="form.name" :label="$t('form.tableInsertUpdate.name')" :rules="[ val => val && val.length > 0 || 'Please type something']" hint=""/>
          <q-select class="col-12 col-md-6" outlined v-model="form.datasource" emit-value map-options option-value="id" :options="datasourceOptions" :label="$t('form.tableInsertUpdate.datasource')" clearable hint="">
          ...
          <q-input class="col-12 col-md-6" outlined v-model="form.schemaName" :label="$t('form.tableInsertUpdate.schema')" :rules="[ val => val && val.length > 0 || 'Please type something']" hint=""/>
          <q-input class="col-12 col-md-6" outlined v-model="form.tableName" :label="$t('form.tableInsertUpdate.table')" :rules="[ val => val && val.length > 0 || 'Please type something']" hint="">
        ...
        <q-tab-panel name="lookup">
          <q-table :data="form.searchMappingData" :columns="searchMappingColumns" :rows-per-page-options="[0]" row-key="name" separator="cell" hide-bottom :title="$t('form.tableInsertUpdate.tableField')">
        ...
        <q-tab-panel name="field">
          <q-table :data="form.fieldMappingData" :columns="fieldMappingColumns" :rows-per-page-options="[0]"
                   row-key="name" separator="cell" hide-bottom :title="$t('form.tableInsertUpdate.tableField')">
            <template v-slot:top-right>
    ...
    <q-dialog v-model="selectTables.mode">
      <q-card style="width: 100vw;">
        <q-card-section class="row items-center q-pb-none">
          <div class="text-h6">{{ $t('form.tableInsertUpdate.datasource') }}</div>
          <q-space/>
  ...
  data () {
    return {
      ...
      searchMappingColumns: [
        { name: 'operate', label: this.$t('form.tableInsertUpdate.columnOperate'), filed: 'operate', align: 'right', headerStyle: 'width: 20px' },
        { name: 'target', label: this.$t('form.tableInsertUpdate.columnTargetField'), field: 'target', align: 'left', headerStyle: 'width: 150px;' },
        { name: 'condition', label: this.$t('form.tableInsertUpdate.columnCondition'), field: 'condition', align: 'left', headerStyle: 'width: 100px;' },
        { name: 'source', label: this.$t('form.tableInsertUpdate.columnSourceField'), field: 'source', align: 'left', headerStyle: 'width: 150px;' },
        { name: 'source2', label: this.$t('form.tableInsertUpdate.columnSourceField2'), field: 'source2', align: 'left', headerStyle: 'width: 150px;' }
      ],
      fieldMappingColumns: [
        { name: 'operate', label: this.$t('form.tableInsertUpdate.columnOperate'), filed: 'operate', align: 'right', headerStyle: 'width: 20px' },
        { name: 'target', label: this.$t('form.tableInsertUpdate.columnTargetField'), field: 'target', align: 'left', headerStyle: 'width: 150px;' },
        { name: 'source', label: this.$t('form.tableInsertUpdate.columnSourceField'), field: 'source', align: 'left', headerStyle: 'width: 150px;' },
        { name: 'update', label: this.$t('form.tableInsertUpdate.columnUpdate'), field: 'update', align: 'left', headerStyle: 'width: 50px;' }
      ],
      state: false,
      datasourceOptions: [],
      sourceFields: [],
  ...
  methods: {
    ...
    loadTables () {
      ...
      if (!vm.form.datasource) {
        vm.$q.notify({
          position: 'top',
          message: vm.$t('form.tableInsertUpdate.datasource'),
          color: 'negative'
        })
      } else {
