<template>
  <div style="width: 100%; margin-left: 0px;">
    ...
    <q-btn-group outline>
      ...
      <q-btn outline color="primary" :label="$t('button.save')" icon="las la-save" @click="submitForm"/>
    ...
    <div ref="graphContainer" style="width: 100%; height: calc(100vh - 160px);"/>
    <div ref="outlineContainer" style="z-index:1;position:absolute;overflow:hidden;bottom:40px;right:0px;width:160px;height:120px;background:transparent;border-style:solid;border-color:lightgray;"/>
...
import { fetchShell } from 'src/service/kettle/ShellService'
...
export default {
  ...
  mounted () {
    ...
    this.graph.addListener(mxgraph.mxEvent.CELL_CONNECTED,(sender, evt) => {
      const category = vm.$route.path.replace('/etl-design/', '')
      ...
      if (target) {
        ...
        if (category.startsWith('1/')) {
          ...
          if (source.style !== 'JobEntrySpecial') {
            vm.$q.dialog({
              title: vm.$t('confirm.kettle.outputCondition'),
              options: {
                type: 'radio',
                model: outgoings.length > 0 ? 2 : 1,
                items: [
                  {
                    label: vm.$t('confirm.kettle.outputUnconditional'),
                    value: 0
                  },
                  {
                    label: vm.$t('confirm.kettle.outputTrue'),
                    value: 1
                  },
                  {
                    label: vm.$t('confirm.kettle.outputFalse'),
                    value: 2
                  }
                ]
              },
              ...
              sender.insertEdge(parent, null, span, source, target, style)
          ...
          if (node) {
            ...
            vm.$q.dialog({
              title: vm.$t('confirm.kettle.outputMode'),
              ok: {
                outline: true,
                color: 'teal',
                label: vm.$t('confirm.kettle.outputNormal')
              },
              cancel: {
                outline: true,
                color: 'negative',
                label: vm.$t('confirm.kettle.outputAbnormal')
              },
              persistent: true
            }).onOk(() => {
              if (error[target.id]) {
              ...
              console.log(evt.getProperty('edge'))
            ...
            vm.$q.dialog({
              ...
              ok: {
                outline: true,
                color: 'teal',
                label: vm.$t('confirm.kettle.outputDefalut')
              },
              cancel: {
                outline: true,
                color: 'negative',
                label: vm.$t('confirm.kettle.outputConditional')
              },
              persistent: true
            }).onOk(() => {
              const span = document.createElement('span')
    ...
    if (vm.$route.path.startsWith('/etl-design/')) {
      const id = vm.$route.path.replace('/etl-design/', '').replace('1/', '').replace('2/', '')
      if (id) {
        fetchShell(id).then(res => {
          vm.shell = res.data
          if (vm.shell.content) {
            vm.paint(res.data.content)
          }
        }).catch(err => {
          if (err.status === 10002) {
            vm.$q.notify({
